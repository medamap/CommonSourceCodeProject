...
Menu::Menu() {
    ...
    // Device メニューを作成
    int diskId = addNode(deviceId, "Disk", Category, -1);
    int soundId = addNode(deviceId, "Sound", Category, -1);

    // Disk メニューを作成
    addNode(diskId, "None", Property, ID_VM_DEVICE_TYPE0);
    addNode(diskId, "Intelligent RAM Disk", Property, ID_VM_DEVICE_TYPE1);
    addNode(diskId, "Nonintelligent RAM Disk", Property, ID_VM_DEVICE_TYPE2);
