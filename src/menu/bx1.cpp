...
Menu::Menu() {
    ...
    int fdId = addNode(rootId, "FD", Category, -1, MENU_FDD0);
    ...
    // FD メニューを作成
    addNode(fdId, "Insert", Property, ID_OPEN_FD1);
    addNode(fdId, "Eject", Property, ID_CLOSE_FD1);
    addNode(fdId, "Insert Blank 1S Disk", Property, ID_OPEN_BLANK_2D_FD1);
    addNode(fdId, "Write Protected", Property, ID_WRITE_PROTECT_FD1);
    addNode(fdId, "Ignore CRC Errors", Property, ID_IGNORE_CRC_FD1);
    addNode(fdId, "Recent", Property, ID_RECENT_FD1);

    // Device メニューを作成
    int modeSwitchId = addNode(deviceId, "Mode Switch", Category, -1);
    int soundId = addNode(deviceId, "Sound", Category, -1);
    int printerId = addNode(deviceId, "Printer", Category, -1);

    addNode(modeSwitchId, "AUTO PRINT Switch", Property, ID_VM_DIPSWITCH0);
    addNode(modeSwitchId, "PROG.SELECT Switch", Property, ID_VM_DIPSWITCH1);
    addNode(modeSwitchId, "JPN/ENG Jumper", Property, ID_VM_DIPSWITCH2);
