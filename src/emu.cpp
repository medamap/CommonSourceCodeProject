...
// Medamap and Claude: Apple プラットフォーム用コンストラクタ
EMU::EMU()
#else
EMU::EMU()
#endif
{
...
#ifdef __APPLE__
	// Apple プラットフォーム: ConfigManagerを使用してconfigを初期化
	// ConfigManagerを初期化（まだ初期化されていない場合）
	ConfigManager& configManager = ConfigManager::getInstance();
	if (!configManager.hasKey("sound_frequency")) {
		// TODO: Remove debug output after ConfigManager stabilization
	...
	config.sound_frequency = configManager.getInt("sound_frequency", 5);  // デフォルト インデックス5 (44100Hz)
	config.sound_latency = configManager.getInt("sound_latency", 1);      // デフォルト インデックス1 (100ms)
	
	// TODO: Remove debug output after ConfigManager stabilization
	#ifdef DEBUG_CONFIG_MANAGER
	printf("[DEBUG] ConfigManagerから設定読み込み: sound_frequency=%d, sound_latency=%d\n", 
	       config.sound_frequency, config.sound_latency);
	fflush(stdout);  // 強制的に出力
	#endif
#endif
	...
	// TODO: Remove debug output after ConfigManager stabilization
	#ifdef DEBUG_CONFIG_MANAGER
	printf("[DEBUG] 音声設定チェック前: config.sound_frequency = %d\n", config.sound_frequency);
	fflush(stdout);
	#endif
	
	if(!(0 <= config.sound_frequency && config.sound_frequency < 8)) {
		config.sound_frequency = 5;	// default: 44100Hz (better compatibility)
		// TODO: Remove debug output after ConfigManager stabilization
		#ifdef DEBUG_CONFIG_MANAGER
		printf("[DEBUG] 音声周波数をデフォルト値に設定: %d (44100Hz)\n", config.sound_frequency);
		#endif
	}
	...
	// TODO: Remove debug output after ConfigManager stabilization
	#ifdef DEBUG_CONFIG_MANAGER
	printf("[DEBUG] 最終音声設定: index=%d, rate=%dHz, samples=%d\n", 
	       sound_frequency, sound_rate, sound_samples);
	fflush(stdout);
	#endif
#if defined(__ANDROID__)
	LOGI("sound Frequency %d: rate %d: samples %d ",config.sound_frequency,sound_rate,sound_samples);
#elif defined(__APPLE__)
	// Medamap and Claude: Apple プラットフォーム用音声設定ログ
	printf("Apple音声設定: Frequency %d, rate %d, samples %d\n", config.sound_frequency, sound_rate, sound_samples);
#endif
#ifdef USE_CPU_TYPE
    ...
    osd = new OSD(state);
#elif defined(__APPLE__)
	// Medamap and Claude: Apple プラットフォーム用OSD初期化
	osd = new OSD();
#else
	osd = new OSD();
#endif
...
#elif defined(__APPLE__)
	// Medamap and Claude: Apple プラットフォーム用追加設定
	// 将来的にはMetalView、AudioSession、ViewController等を設定可能
	printf("Apple プラットフォーム用OSD初期化完了\n");
#endif
